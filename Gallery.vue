    ...
    <div ref="view" v-show="visible">
        <WrapVue :noTransition="wrapNoTransition" :opacity="wrapOpacity" :showWrap="showWrap" @onCloseWrap="onClose">
            <div @dragstart='$event.preventDefault();' ref="mediaRef" class="media-content">
                    <img :id="imageIds[index]" :style="index===imageAttr.index?imageAttr.style:otherImgStyle(index)" 
                        :class="[{'img-transition': transition.img.enable}]" :ref="'imgRefs'+index" 
                        :key="index" v-for="(item, index) in mediaDatas" 
                        :src="item" v-if="isCreated(index)" @load="imgLoaded(index, $event)" alt="image load failed"/>
...
export default {
    ...
    mounted(){
        ...
        gallery.on('slide', obj=>{
            ...
            this.slideTransition = this.renderer(null, slide, 0);
        });
        ...
        gallery.on('swipe', obj=>{
            ...
            if(slide){
                ...
                slide = slide.slice();
                this.resetOtherImgAttr(200);
                this.renderer(null, slide, 200).end(()=>{
                    ...
                    if(correctResult.correct){
                        this.renderer(null, correctResult.slide, 260);
                    }
                });
            }
        });
    ...
    methods: {
        ...
        imgLoaded(index, event){
            ...
            if(index === this.imageAttr.index){
                this.showImage(true, index).then(()=>{
                    this.gallery.setImageLoaded(index);
                });
            }else{
                this.gallery.setImageLoaded(index);
            }
        ...
        showImage(bool, index){
            return new Promise((resolve, reject)=>{
                if(bool){
                    ...
                    if(!size){return;}
                    setTimeout(() => {
                        this.renderer({x: size[0], y: size[1]}, null, 200).end(()=>{
                            resolve();
                        });
                    }, 10);
                } else {
                    let size = this.imageAttr.sizes[this.imageAttr.index];
                    this.renderer({x: 0-this.imageAttr.width, y: 0-this.imageAttr.height}, null, size?200:0).end(()=>{
        ...
        imageSwitch(switchObj){
            ...
            this.doImgTransition(()=>{
                ...
                let curImgSize = this.imageAttr.sizes[this.imageAttr.index]||[0, 0];
                this.translate = {x: 0, y: 0};
                this.imageAttr.width = curImgSize[0];
                this.imageAttr.height = curImgSize[1];
                this.imageAttr.style = this.buildImageStyle({x: curImgSize[0], y: curImgSize[1]}, this.translate);
        ...
        renderer(size, slide, duration){
            ...
            return new Transition(datas=>{
                datas.forEach(dataObj=>{
                    this.imageAttr[dataObj.type] += dataObj.number;
                });
    ...
    computed: {
        translate: {
            set(newVal){
                this.imageAttr.translateX = newVal.x;
                this.imageAttr.translateY = newVal.y;
            },
            get(){
